...
var user_settings_lobby_url_input: LineEdit
var user_settings_access_token_input: LineEdit
...
func _ready() -> void:
	load_user_settings()
	_load_local_development_lobby_config()
...
func create_user_settings_ui() -> void:
	...
	user_settings_lobby_url_input = $UIRoot/Home/UserSettingsModal/Panel/LobbyUrlInput
	user_settings_access_token_input = $UIRoot/Home/UserSettingsModal/Panel/InviteTokenInput
...
func open_user_settings() -> void:
	# Controls with later scene-tree order can receive mouse input before this
	# modal, even when the modal has a higher z-index. Move it to the front and
	# disable every Home action while settings are open so Save always owns its
	# click instead of Debug or another background action.
	user_settings_modal.move_to_front()
	_set_home_buttons_blocked_for_user_settings(true)
	user_settings_name_input.text = user_display_name
	user_settings_lobby_url_input.text = lobby_api_url
	user_settings_access_token_input.text = lobby_access_token
...
func close_user_settings() -> void:
	if user_settings_modal:
		user_settings_modal.visible = false
	_set_home_buttons_blocked_for_user_settings(false)


func _set_home_buttons_blocked_for_user_settings(blocked: bool) -> void:
	if blocked:
		if not user_settings_home_button_states.is_empty():
			return
		for path in USER_SETTINGS_HOME_BUTTON_PATHS:
			var button := get_node_or_null(NodePath(path)) as Button
			if button == null:
				continue
			user_settings_home_button_states[path] = {
	...
	for path in USER_SETTINGS_HOME_BUTTON_PATHS:
		var button := get_node_or_null(NodePath(path)) as Button
		var state: Dictionary = user_settings_home_button_states.get(path, {})
		if button == null or state.is_empty():
			continue
		button.disabled = bool(state["disabled"])
		button.mouse_filter = int(state["mouse_filter"])
	user_settings_home_button_states.clear()
...
func load_user_settings() -> void:
	...
	lobby_api_url = str(config.get_value("network", "lobby_url", lobby_api_url)).strip_edges().trim_suffix("/")
	lobby_access_token = str(config.get_value("network", "invite_token", lobby_access_token)).strip_edges()


func _load_local_development_lobby_config() -> void:
	# This file is generated by start-local.ps1, ignored by Git, and excluded
	# from release exports. It restores the former one-command local workflow
	# without embedding a development token in distributed clients.
	if not OS.is_debug_build() or not lobby_api_url.is_empty() or not lobby_access_token.is_empty():
		return
	...
	while not file.eof_reached():
		...
		if separator > 0:
			values[line.left(separator)] = line.substr(separator + 1).strip_edges()
	var candidate_url := str(values.get("LOBBY_URL", "")).trim_suffix("/")
	var candidate_token := str(values.get("INVITE_TOKEN", "")).strip_edges()
	if not _is_loopback_http_lobby_url(candidate_url):
		return
	var previous_allow_insecure := allow_insecure_lobby_url
	allow_insecure_lobby_url = true
	if _lobby_configuration_error(candidate_url, candidate_token).is_empty():
		lobby_api_url = candidate_url
		lobby_access_token = candidate_token
		return
	allow_insecure_lobby_url = previous_allow_insecure
...
func save_user_settings() -> void:
	...
	if not MatchProtocol.valid_display_name(candidate):
		...
		return
	var candidate_lobby_url := user_settings_lobby_url_input.text.strip_edges().trim_suffix("/")
	var candidate_access_token := user_settings_access_token_input.text.strip_edges()
	if not candidate_lobby_url.is_empty() or not candidate_access_token.is_empty():
		var network_error := _lobby_configuration_error(candidate_lobby_url, candidate_access_token)
		if not network_error.is_empty():
			user_settings_validation.text = network_error
			return
	user_display_name = candidate
	lobby_api_url = candidate_lobby_url
	lobby_access_token = candidate_access_token
	var config := ConfigFile.new()
	config.set_value("user", "display_name", user_display_name)
	config.set_value("network", "lobby_url", lobby_api_url)
	config.set_value("network", "invite_token", lobby_access_token)
	...
	if dedicated_connection:
		dedicated_connection.configure_lobby(lobby_api_url, lobby_access_token, allow_insecure_lobby_url)
	if dedicated_connection and dedicated_connection.has_pending_join():
		_send_dedicated_loadout(p1_selection if local_player_id == 1 else p2_selection)
	refresh_lobby_label()
	# Button.pressed is emitted while its mouse-release event is still being
	# dispatched. Keep the modal input barrier alive until that event completes
	# so a button behind the modal cannot receive the same click.
	call_deferred("close_user_settings")


func _lobby_configuration_error(url: String, token: String) -> String:
	var normalized_url := url.strip_edges().trim_suffix("/")
	...
	if token.strip_edges().length() < 32 or token.contains(" "):
...
func show_connection() -> void:
	...
	if not configuration_error.is_empty():
		network_status_label.text = configuration_error
